...
AIEN.config.artyTaskTimeout                   = 120 -- 240
...
local resupplyMaxDist                   = 5000            -- m of distance for the maximum allowed resupply route.
...
underAttack = underAttack or {}                -- used when a group has been attacked, it keeps "tactical" tasking off for 10 mins leaving room for "reaction" decision making
...
function rndInZone(z)
  local d = trigger.misc.getZone(z)
  if not d then 
    env.info("[AIEN] rndInZone: trigger.misc.getZone(" .. tostring(z) .. ") returned nil")
    return nil 
  end
  local r = d.radius * math.sqrt(math.random())
  local a = math.random() * 6.28318
  local x = d.point.x + r * math.cos(a)
  local z = d.point.z + r * math.sin(a)
...
local function getRandTerrainPointInCircle(var, radius, innerRadius, requestV3)
    ...
    if not FORCE_FALLBACK then
        local zd    = bc:getZoneOfPoint(point)
        local cz    = zd and zd.zone and CustomZone:getByName(zd.zone)
        if cz then
...
local function hasTargets(group, report)
	if group and group:isExist() == true then
		...
		if table.getn(tblUnits) > 0 then
			...
			env.info(("AIEN.hasTargets: tblUnits has 0 units"))
			return false			
		end
	else
		env.info(("AIEN.hasTargets: group is nil"))
		return false	
	end	
end
        ...
        if gController and vec3 then
            ...
            if AIEN.config.message_feed == true then
                ...
                if lat and lon then
                    ...
                    txt = txt .. "C2, " .. shortName .. ", request fire mission, fire for Effect. coordinates:"
        ...
        if unit1 then
            ...
            if point then	
                ...
                if not rndCoord then
                    if useZone == false then
                        env.info('UseZone is false')
                ...
                if rndCoord then
                    ...
                    if posStart then
                        ...
                        goRoute(group, path)
                        -- SAMs need to be GREEN for a short time so they start moving
                        -- if unit1 and ( unit1:hasAttribute("SAM SR")
                        --     or unit1:hasAttribute("SAM TR")
                        --     or unit1:hasAttribute("SAM CC")
                        --     or unit1:hasAttribute("SAM LL") ) then
                        --     env.info((tostring(ModuleName) .. ", moveToPoint SAM group detected, setting alarm state to green"))

                        --     local ctrl = group:getController()
                        --     Controller.setOption(ctrl, AI.Option.Ground.id.ALARM_STATE, AI.Option.Ground.val.ALARM_STATE.GREEN)

                        --     local function _rst()
                        --         if group and group:isExist() then
                        --             Controller.setOption(group:getController(),
                        --                 AI.Option.Ground.id.ALARM_STATE,
                        --                 AI.Option.Ground.val.ALARM_STATE.AUTO)
                        --         end
                        --     end
                        --     timer.scheduleFunction(_rst, nil, timer.getTime() + 30)
                        -- end                     
...
local function orderInfantryToMoveToPoint(_group, _destination)
    ...
    if _zc then
        local _cz = CustomZone:getByName(_zc.zone)
        if _cz then
...
local function deployTroops(unit, exactPos)
    ...
    local zoneCom = BattleCommander:getZoneOfUnit(unit:getName())
        if not zoneCom or zoneCom.side ~= 1 then
        return
    end
    ...
    if group and group:isExist() and ownPos and sa then
        ...
        local funcDoAction = function()
            ...
            if group:isExist() then
                ...
                if smoked == true then
                    moveToPoint(group, ownPos, 5, 14, nil, nil, nil, nil, nil, nil, false) -- 5,14
                    if AIEN.config.AIEN_debugProcessDetail == true then
...
local function ac_withdraw(group, ownPos, tgtPos, resume, sa, skill) -- this will make the group to run away to the nearest allied ground group
    ...
    if group and ownPos then
                ...
                if og.n ~= group:getName() then
                    if og.group and og.group:isExist() == true and og.sa and og.sa.pos then
                        ...
                        if p then -- and td
                            ...
                            if d and d < maxDist and d > 1000 then
                                bestPos = p
                                maxDist = d
        ...
        if bestPos then 
            local funcDoAction = function()
                if group:isExist() then
                    moveToPoint(group, bestPos, AIEN.config.repositionDistance*1.5, AIEN.config.repositionDistance*0.5) 
                end
            end
            if AIEN.config.AIEN_debugProcessDetail == true then
                ...
                if group:isExist() then
                    ...
                    local funcresumeRoute = function()
                        if group:isExist() then
                            ...
                            if z1 and dZone and z1.zone == dZone.zone then
    ...
    if group and tgtPos then
        ...
        local funcDoAction = function()
            if group:isExist() then
                ...
                moveToPoint(group, tgtPos, 300, 500, false, "cone", nil, nil, nil, speed,false) 
            end
        end
        if AIEN.config.AIEN_debugProcessDetail == true then
...
local function ac_coverBuildings(group, ownPos, tgtPos, resume, sa, skill)
    ...
    if group and ownPos and sa then
        ...
        local cZone = CustomZone:getByName(z1.zone)
        if not cZone then
            if AIEN.config.AIEN_debugProcessDetail then
        ...
        if AllyGroup and bestTd then 
            local funcDoAction = function()
                if AllyGroup:isExist() then
                     moveToPoint(AllyGroup, ownPos, 150, 40, nil, nil, nil, nil, nil, nil, false)
                end
            end
            if AIEN.config.AIEN_debugProcessDetail == true then
...
local function ac_coverADS(group, ownPos, tgtPos, resume, sa, skill) -- this will make the group to run into the effective range of an allied air defence group
    ...
    if group and ownPos and sa then
        ...
        if bestPos and bestTd then 
            ...
            local funcDoAction = function()
                if group:isExist() then
                    moveToPoint(group, bestPos, 100,30, false, nil, nil, nil, nil, nil, false)
                end
            end
            if AIEN.config.AIEN_debugProcessDetail == true then
		...
		for _, gp in pairs(coalition.getGroups(i,2)) do -- ground only
			if gp:isExist() then
                ...
                if c then
                    ...
                    groundgroupsDb[gp:getID()] = {group = gp, class = c, n = gp:getName(), coa = gpcoa, detection = det, threat = thr, tasked = false, skill = s, sa = {}}  --, route = r
            ...
            if not initiator:hasAttribute("Infantry") then
                ...
                if gp then
                    if not groundgroupsDb[gp:getID()] then -- since event is launched for each unit, this prevent re-adding the same group multiple times
                        ...
                        groundgroupsDb[gp:getID()] = {group = gp, class = c, n = gp:getName(), coa = gp:getCoalition(), detection = det, threat = thr, tasked = false, skill = s, sa = {} }
                        if c == "ARTY" or c == "MLRS" then
                            env.info(("ARTYDBG "..tostring(gp:getName())))
                            phase_keys = createIterator(groundgroupsDb)
                        end                      
                    end
                end
